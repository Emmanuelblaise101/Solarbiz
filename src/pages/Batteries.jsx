...
const Batteries = () => {
  ...
  const batteries = productData.batteries || [];
  return (
        ...
        <div className="container mx-auto px-8 relative z-10 grid md:grid-cols-2 gap-12 items-center">
          <div>
            <h1 className="font-headline text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tighter text-on-surface mb-6 uppercase break-words">
              DEEP CYCLE. <br/><span className="text-primary-container">BUILT DIFFERENT.</span>
            </h1>
            ...
            <p className="font-body text-xl text-on-surface-variant max-w-xl mb-10 leading-relaxed">
              Industrial-grade energy storage engineered for Nigerian power conditions. Built to absorb, hold, and deliver through every load shedding cycle.
            </p>
          </div>
      ...
      <section className="py-24 bg-surface-container-low relative">
        <div className="container mx-auto px-8">
          <div className="mb-16">
            ...
            <h2 className="font-headline text-4xl font-bold tracking-tight">Inverter Batteries</h2>
          </div>
          ...
          {loading ? (
            ...
            {batteries.map((battery) => (
              <div key={battery.id} className="bg-surface-container-highest rounded-lg p-1 group border border-outline-variant/10 hover:border-primary/30 transition-all">
                <div className="bg-surface-container h-full w-full p-8 flex flex-col justify-between">
                  <div>
                    <div className="inline-flex bg-tertiary-container/20 text-tertiary px-3 py-1 rounded-full text-xs font-label mb-6">{battery.badge}</div>
                    <h3 className="font-headline text-2xl font-bold mb-3 uppercase tracking-tight">{battery.name}</h3>
                    <p className="font-body text-sm text-on-surface-variant mb-6">{battery.description}</p>
                    
                    <ul className="space-y-3 mb-8">
                      <li className="flex justify-between border-b border-white/5 pb-2">
                        <span className="font-label text-xs text-neutral-500 uppercase">{battery.spec1Label}</span>
                        <span className="font-label text-xs text-on-surface">{battery.spec1Value}</span>
                      </li>
                      <li className="flex justify-between border-b border-white/5 pb-2">
                        <span className="font-label text-xs text-neutral-500 uppercase">{battery.spec2Label}</span>
                        <span className="font-label text-xs text-on-surface">{battery.spec2Value}</span>
                      </li>
                    </ul>
                  </div>
                  <div>
                    ...
                    <button 
                      onClick={() => addToCart({ id: battery.id, name: battery.name, price: battery.price, image: battery.image })}
